...
-- 宝石熔炼只允许在设有熔炼NPC的场景进行
x800118_g_MeltingNPCTbl =
{
	[0]   = "洛阳（178，185）彭怀玉",
	[420] = "束河古镇（134，84）荆嵌实",
	[186] = "楼兰（74，161）克里木",
...
function x800118_OnGemMelting( sceneId, selfId, GemItemPos1, GemItemPos2, GemItemPos3, NeedItemPos )
	...
	-- 非法场景直接拒绝，防止伪造封包跨场景熔炼宝石
	local NPCInfo = x800118_g_MeltingNPCTbl[sceneId]
	if NPCInfo == nil then
		x800118_NotifyTip( sceneId, selfId, "当前场景无法进行宝石熔炼。" )
		return
	end
	...
	--检测是否可以熔炼....
	local ProductID, NeedID, NeedMoney, NeedID2 = LuaFnGetGemMeltingInfo( GemItemID1 )
	if  -1 == ProductID then
	...
	--检测是否有所需物品....
	if NeedID ~= NeedItemID and (NeedID2 == nil or NeedID2 == -1 or NeedID2 ~= NeedItemID) then
		local NeedItemMsg = "#{JKBS_081021_010}#{_ITEM" .. NeedID .. "}"
		if NeedID2 ~= nil and NeedID2 ~= -1 then
			NeedItemMsg = NeedItemMsg .. " / #{_ITEM" .. NeedID2 .. "}"
		end
		x800118_NotifyTip( sceneId, selfId, NeedItemMsg )
		return
	end
