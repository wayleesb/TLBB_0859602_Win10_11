...
x050102_g_LimitMembers = LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT					-- 可以进副本的最小队伍人数
...
function x050102_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050102_g_MissionId ) > 0 then					-- 有任务才可以走这里
		...
		if nearMemberCount < x050102_g_LimitMembers then
			x050102_NotifyFailBox( sceneId, selfId, targetId, "    剿贼一事非同小可，你需要一些本领与你相当的帮手我才放心。（队伍中至少需要"..LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT.."个30级以上角色）" )
			return
		end
...
function x050102_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < x050102_g_LimitMembers then
		x050102_NotifyFailTips( sceneId, leaderObjId, "    剿贼一事非同小可，你需要一些本领与你相当的帮手我才放心。（队伍中至少需要"..LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT.."个30级以上角色）" )
		return
	end
