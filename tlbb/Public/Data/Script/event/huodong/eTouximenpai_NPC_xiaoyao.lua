...
x808016_g_LimitMembers			= TOUXIMENPAI_MIN_ENTRY_PLAYER_COUNT		--可以进副本的最小队伍人数
...
x808016_g_keySD["killedmonsternum"]	= 9		--杀死喽罗的数量
...
function x808016_OnEnumerate( sceneId, selfId, targetId )
	...
	if(TOUXIMENPAI_ENABLE_MENPAI_RESTRICTION == 1 and MenPaiID ~= x808016_g_MenPaiID) then
	...
	if GetTeamSize( sceneId, selfId ) < x808016_g_LimitMembers then
	  x808016_NotifyTip( sceneId, selfId, "想要挑战我至少也得上来"..TOUXIMENPAI_MIN_ENTRY_PLAYER_COUNT.."个吧，就这点人？也太瞧不起我了。" )
	  return
	end
	--PrintNum(4)
