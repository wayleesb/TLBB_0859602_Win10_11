...
x808121_tipinbox = 1
x808121_g_limitMembers = TIANJIANG_QISHOU_MIN_ENTRY_PLAYER_COUNT        --人数限制
...
function x808121_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1 then
		...
		local strOutmsg = "";
		if (nCheckRet == 1) then
			strOutmsg = "想挑战奇兽，你需要一支"..TIANJIANG_QISHOU_MIN_ENTRY_PLAYER_COUNT.."人或以上的队伍。";
		elseif (nCheckRet == 2) then
			strOutmsg = "你不是队长，请让你的队长来找我。";
		elseif (nCheckRet == 3) then
			strOutmsg = "想挑战奇兽，你需要一支"..TIANJIANG_QISHOU_MIN_ENTRY_PLAYER_COUNT.."人或以上的队伍。"           --队伍人数不足
