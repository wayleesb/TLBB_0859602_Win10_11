...
x050220_g_LimitMembers = XINSANHUAN_MIN_ENTRY_PLAYER_COUNT					-- 可以进副本的最小队伍人数
...
function x050220_OnDie( sceneId, objId, selfId )--这个selfId可能是宠物
	...
	-- 前置怪物全部击杀后立即刷新王阎，计时器仅作为15分钟兜底。
	if x050220_IsBossWangReady( sceneId ) == 1 then
		x050220_LoadBossWang( sceneId )
	end
end
...
-- 前置条件满足时只允许创建一次王阎，避免击杀回调和计时器重复刷怪。
function x050220_IsBossWangReady( sceneId )
	if LuaFnGetCopySceneData_Param( sceneId, 10 ) > 0 then
		return 0
	end

	if LuaFnGetCopySceneData_Param( sceneId, 7 ) < 60 then
		return 0
	end
	if LuaFnGetCopySceneData_Param( sceneId, 8 ) ~= 1
		or LuaFnGetCopySceneData_Param( sceneId, 9 ) ~= 1 then
		return 0
	end
...
function x050220_LoadBossWang( sceneId )
	if x050220_IsBossWangReady( sceneId ) ~= 1 then
		return
	end
	...
	if BossWangId <= 0 then
		return
	end

	local monsterID = LuaFnCreateMonster( sceneId, BossWangId, 200, 48, 14, 262, 050220 )
	if monsterID == -1 or LuaFnIsObjValid( sceneId, monsterID ) ~= 1 then
		return
	end
...
--				LuaFnSetCopySceneData_Param( sceneId, 11, TickCount )
		...
		--15分钟仍未完成前置时不刷，完成后的下一次计时回调作为兜底。
		elseif TickCount >= 180 then
			x050220_LoadBossWang( sceneId )
		end
