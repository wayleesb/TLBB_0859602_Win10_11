...
x050221_g_LimitMembers = XINSANHUAN_MIN_ENTRY_PLAYER_COUNT					-- 可以进副本的最小队伍人数
...
function x050221_OnDie( sceneId, objId, selfId )--这个selfId可能是宠物
	...
	for i = 1, getn( x050221_g_BossID ) do
		if szName == x050221_g_BossID[i] then
			...
			-- 五个前置 Boss 全部击杀后立即生成洪棘妖王，避免无意义等待。
...
function x050221_IsBossYaoWangReady( sceneId )
	...
	-- 已引出的 Boss 仍存活时不能提前生成洪棘妖王。
	local monsterCount = GetMonsterCount( sceneId )
	for i = 0, monsterCount - 1 do
		local monsterID = GetMonsterObjID( sceneId, i )
		if LuaFnIsObjValid( sceneId, monsterID ) == 1 and LuaFnIsCharacterLiving( sceneId, monsterID ) == 1 then
			local monsterName = GetName( sceneId, monsterID )
			for j = 1, getn( x050221_g_BossID ) do
				if monsterName == x050221_g_BossID[j] then
					return 0
				end
			end
		end
...
-- 生成洪棘妖王；force=1 时保留原有 15 分钟兜底逻辑。
function x050221_LoadBossYaoWang( sceneId, force )
	...
	if force ~= 1 and x050221_IsBossYaoWangReady( sceneId ) ~= 1 then
		return
	end
