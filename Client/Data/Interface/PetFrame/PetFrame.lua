...
function PetFrame_Update()
	...
	PetFrame_HP:SetProgress(PET_MAXHP > 0 and math.min(1, math.max(0, PET_HP / PET_MAXHP)) or 0, 1);
	if( PET_MAXHP > 0 and PET_HP / PET_MAXHP > 0.3 ) then
	...
	PetFrame_Exp:SetProgress(PET_MAXEXP > 0 and math.min(1, math.max(0, PET_EXP / PET_MAXEXP)) or 0, 1);	
...
function PetFrame_Update_Tooltip( arg )
	PetFrame_HP:SetToolTip("血:"..string.format("%.0f", PET_HP) .. "/" .. string.format("%.0f", PET_MAXHP).."#r".."经验:"..string.format("%.0f", PET_EXP).."/"..string.format("%.0f", PET_MAXEXP) );
	PetFrame_Exp:SetToolTip("血:"..string.format("%.0f", PET_HP) .. "/" .. string.format("%.0f", PET_MAXHP).."#r".."经验:"..string.format("%.0f", PET_EXP).."/"..string.format("%.0f", PET_MAXEXP) );
