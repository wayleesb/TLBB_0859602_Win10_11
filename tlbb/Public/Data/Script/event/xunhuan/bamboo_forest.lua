...
x050101_g_LimitMembers = LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT					-- 可以进副本的最小队伍人数
...
function x050101_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050101_g_MissionId ) > 0 then					-- 有任务才可以走这里
		...
		if nearMemberCount < x050101_g_LimitMembers then
			x050101_NotifyFailBox( sceneId, selfId, targetId, "    竹林里野熊很多，最少你们得有 "..LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT.." 位同伴我才放心让你们去。（队伍中至少需要"..LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT.."个30级以上角色）" )
			return
		end
...
function x050101_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < x050101_g_LimitMembers then
		x050101_NotifyFailTips( sceneId, leaderObjId, "    竹林里野熊很多，最少你们得有 "..LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT.." 位同伴我才放心让你们去。" )
		return
	end
