...
x401040_g_LimitMembers = YANZIWU_MIN_ENTRY_PLAYER_COUNT				--可以进副本的最小队伍人数
...
function x401040_OnDefaultEvent( sceneId, selfId, targetId )
	...
	-- 根据玩家的情况，将玩家送到不同的副本
	-- 2，检测玩家是不是组队了
	if GetTeamSize(sceneId,selfId) < YANZIWU_MIN_ENTRY_PLAYER_COUNT  then
		BeginEvent(sceneId)
			AddText(sceneId,"#B讨伐燕子坞");
			AddText(sceneId,"  此战凶险异常，不足"..YANZIWU_MIN_ENTRY_PLAYER_COUNT.."人我可不敢让你们进入。");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
