...
-- 配方等级(需求技能的等级)
x700920_g_RecipeLevel = 4

-- 材料表
x700920_g_CaiLiao = {
	{ID = 20105004, Count = 11},
	{ID = 20107004, Count = 13},
	{ID = 20108100, Count = 14},
	{ID = 20308074, Count = 1},
}
...
function x700920_AbilityProduce(sceneId, selfId)
	...
	for i, item in x700920_g_ChanPin do
		if item.Odds >= rand then
			Quality = CallScriptFunction( ABILITYLOGIC_ID, "CalcQuality", sceneId, x700920_g_RecipeLevel, AbilityLevel, x700920_g_AbilityMaxLevel,item.ID )
			...
			LuaFnAuditAbility( sceneId, selfId, x700920_g_AbilityID, x700920_g_RecipeID, Quality )
			LuaFnSendAbilitySuccessMsg( sceneId, selfId, x700920_g_AbilityID, x700920_g_RecipeID, item.ID )
			return OR_OK
		end
		LuaFnAuditAbility( sceneId, selfId, x700920_g_AbilityID, x700920_g_RecipeID, 0 )
	end
