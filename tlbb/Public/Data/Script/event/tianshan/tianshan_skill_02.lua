...
x228903_g_LimitMembers = TIANSHAN_SKILL_02_MIN_ENTRY_PLAYER_COUNT		--可以进副本的最小队伍人数
...
function x228903_AcceptEnterCopyScene( sceneId, selfId )
	if IsHaveMission( sceneId, selfId, x228903_g_MissionId ) > 0 then					--有任务才可以走这里
		...
		if nearteammembercount < x228903_g_LimitMembers then
			x228903_NotifyFailTips( sceneId, selfId, "队伍人数不足，至少需要 " .. x228903_g_LimitMembers .. " 人。" )
			return
		end
...
function x228903_OnCopySceneReady( sceneId, destsceneId )
	...
	if nearteammembercount < x228903_g_LimitMembers then
		x228903_NotifyFailTips( sceneId, leaderObjId, "队伍人数不足，至少需要 " .. x228903_g_LimitMembers .. " 人。" )
		return
	end
