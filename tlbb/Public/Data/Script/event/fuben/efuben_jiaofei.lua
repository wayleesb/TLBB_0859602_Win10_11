...
x402030_g_LimitMembers = JINGHU_JIAOFEI_MIN_ENTRY_PLAYER_COUNT				--可以进副本的最小队伍人数
...
x402030_g_MinHumanCount = JINGHU_JIAOFEI_MIN_ENTRY_PLAYER_COUNT
...
function x402030_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if GetTeamSize(sceneId,selfId) < x402030_g_MinHumanCount  then
		BeginEvent(sceneId)
			AddText(sceneId,"#B镜湖剿匪");
			AddText(sceneId,"  镜湖水匪人多势众，阁下单枪匹马实在太危险了，建议最少"..JINGHU_JIAOFEI_MIN_ENTRY_PLAYER_COUNT.."个人组队前来方可参加。");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
