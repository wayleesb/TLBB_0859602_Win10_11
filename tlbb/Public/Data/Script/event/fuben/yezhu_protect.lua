...
x402102_g_LimitMembers = BAOZOU_YEZHU_MIN_ENTRY_PLAYER_COUNT				--可以进副本的最小队伍人数
...
function x402102_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnHasTeam(sceneId,selfId) < 1  then
		BeginEvent(sceneId)
			...
			AddText(sceneId,"   <灵兽焦急的向你眨了三下眼，似乎想说：不够"..BAOZOU_YEZHU_MIN_ENTRY_PLAYER_COUNT.."个人，你们这不是白白送死吗！>#R(需要是队长，并且队伍中至少"..BAOZOU_YEZHU_MIN_ENTRY_PLAYER_COUNT.."人)");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	-- 2，检测队伍是不是够人数
	if GetTeamSize(sceneId,selfId) < BAOZOU_YEZHU_MIN_ENTRY_PLAYER_COUNT  then
		BeginEvent(sceneId)
			AddText(sceneId,"#B拯救灵兽");
			AddText(sceneId,"   <灵兽焦急的向你眨了三下眼，似乎想说：不够"..BAOZOU_YEZHU_MIN_ENTRY_PLAYER_COUNT.."个人，你们这不是白白送死吗！>#R(需要是队长，并且队伍中至少"..BAOZOU_YEZHU_MIN_ENTRY_PLAYER_COUNT.."人)");
		EndEvent(sceneId)
