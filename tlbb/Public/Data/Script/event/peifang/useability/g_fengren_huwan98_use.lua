...
-- 缝纫 8 级
-- 该脚本有以下三个必备功能函数：
-- x700957_AbilityCheck ——技能使用检查函数
-- x700957_AbilityConsume ——合成结束，进行相关消耗
-- x700957_AbilityProduce ——合成成功，产出产品
...
-- 配方等级(需求技能的等级)
x700957_g_RecipeLevel = 12

-- 材料表
x700957_g_CaiLiao = {
	{ID = 20105012, Count = 22},
	{ID = 20107018, Count = 24},
	{ID = 20108096, Count = 37},
	{ID = 20308090, Count = 1},
}
...
function x700957_AbilityProduce(sceneId, selfId)
	...
	for i, item in x700957_g_ChanPin do
		if item.Odds >= rand then
			Quality = CallScriptFunction( ABILITYLOGIC_ID, "CalcQuality", sceneId, x700957_g_RecipeLevel, AbilityLevel, x700957_g_AbilityMaxLevel,item.ID )
			...
			LuaFnAuditAbility( sceneId, selfId, x700957_g_AbilityID, x700957_g_RecipeID, Quality )
			LuaFnSendAbilitySuccessMsg( sceneId, selfId, x700957_g_AbilityID, x700957_g_RecipeID, item.ID )
			return OR_OK
		end
		LuaFnAuditAbility( sceneId, selfId, x700957_g_AbilityID, x700957_g_RecipeID, 0 )
	end
