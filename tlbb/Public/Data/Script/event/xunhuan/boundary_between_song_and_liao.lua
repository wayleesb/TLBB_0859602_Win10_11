...
x050100_g_LimitMembers = LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT					-- 可以进副本的最小队伍人数
...
x050100_g_ViceBossLoadedParam = 16			-- 副都统刷新标记，防止提前刷新与定时刷新重复创建
...
function x050100_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050100_g_MissionId ) > 0 then					-- 有任务才可以走这里
		...
		if nearMemberCount < x050100_g_LimitMembers then
			x050100_NotifyFailBox( sceneId, selfId, targetId, "    剿贼一事非同小可，你需要一些本领与你相当的帮手我才放心。（队伍中至少需要"..LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT.."个30级以上角色）" )
			return
		end
    ...
    local PlayerMaxLevel = GetHumanMaxLevelLimit()
		...
		LuaFnSetCopySceneData_Param( sceneId, x050100_g_ViceBossLoadedParam, 0 )
...
function x050100_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < x050100_g_LimitMembers then
		x050100_NotifyFailTips( sceneId, leaderObjId, "    剿贼一事非同小可，你需要一些本领与你相当的帮手我才放心。（队伍中至少需要"..LAOSANHUAN_MIN_ENTRY_PLAYER_COUNT.."个30级以上角色）" )
		return
	end
...
function x050100_OnKillObject( sceneId, selfId, objdataId, objId )						-- 参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	...
	-- 前置怪物全部消灭后立即刷新，15分钟刷新作为保底
	if x050100_IsViceBossReady( sceneId ) == 1 then
		x050100_LoadViceBoss( sceneId )
	end
...
--副都统刷新逻辑
--**********************************
function x050100_IsViceBossReady( sceneId )
	local demandIndex = { 1, 2, 4 }
	for i = 1, getn( demandIndex ) do
		local index = demandIndex[i]
		if LuaFnGetCopySceneData_Param( sceneId, 7 + index - 1 ) < x050100_g_DemandKill[index].num then
			return 0
		end
	end
...
function x050100_LoadViceBoss( sceneId )
	if LuaFnGetCopySceneData_Param( sceneId, x050100_g_ViceBossLoadedParam ) > 0 then
		return
	end

	-- 副都统未刷新时创建一次
	if LuaFnGetCopySceneData_Param( sceneId, 9 ) >= x050100_g_DemandKill[3].num then
		LuaFnSetCopySceneData_Param( sceneId, x050100_g_ViceBossLoadedParam, 1 )
		return
	end
	...
	for i = 0, monsterCount - 1 do
		...
		if LuaFnIsObjValid( sceneId, monsterId ) == 1
		 and GetMonsterGroupID( sceneId, monsterId ) == x050100_g_ViceBossGroup
		 and LuaFnIsCharacterLiving( sceneId, monsterId ) == 1 then
			LuaFnSetCopySceneData_Param( sceneId, x050100_g_ViceBossLoadedParam, 1 )
			return
		end
	end
	...
	for i = 0, memberCount - 1 do
		local memberId = LuaFnGetCopyScene_HumanObjId( sceneId, i )
		if LuaFnIsObjValid( sceneId, memberId ) == 1 then
			PosX, PosZ = LuaFnGetWorldPos( sceneId, memberId )
			if LuaFnIsTeamLeader( sceneId, memberId ) == 1 then
				break
			end
		end
	end
	...
	if bossId == -1 or LuaFnIsObjValid( sceneId, bossId ) ~= 1 then
		return
	end
...
function x050100_OnCopySceneTimer( sceneId, nowTime )
	...
	elseif TickCount == x050100_g_LimitTotalHoldTime then									-- 副本总时间限制到了
		...
		-- 副本达到15分钟仍未刷新时，在队长玩家身边保底刷出伪装的宋兵副都统
