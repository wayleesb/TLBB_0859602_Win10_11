...
function x889050_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if NumText == 111 then
		BeginEvent(sceneId)
			AddText(sceneId,"    秦皇战神，可助你功力大增。从现在开始，凡等级#G≥30#W级的玩家都可以在我这里领取#G秦皇战神BUFF#W，该BUFF可以提升您的攻击能力噢！您确定要领取吗？")
			AddNumText( sceneId, x889050_g_ScriptId, "确定", 8, 113 )
			AddNumText( sceneId, x889050_g_ScriptId, "取消", 8, 114 )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	  
	elseif NumText == 112 then
		BeginEvent(sceneId)
			AddText(sceneId, "#Y秦皇战神，助你功力大增。#r #r#W    凡等级#G≥30#W级的玩家都可以在我这里领取#G秦皇战神BUFF#W，该BUFF可使内外攻增强#W，持续时间为#G12小时#W，角色死亡后BUFF不消失，下线后持续计时。")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
...
function x889050_OnEnumerate( sceneId, selfId, targetId )
  ...
  --检测时间是否正确
	...
	AddNumText(sceneId, x889050_g_ScriptId, "秦皇战神活动", 6, 111 )
	AddNumText(sceneId, x889050_g_ScriptId, "关于秦皇战神活动", 11, 112 )													
end
...
function x889050_GiveBuff( sceneId, selfId, targetId )
	...
	if	GetLevel( sceneId, selfId) < 30 then
		BeginEvent(sceneId)
			AddText( sceneId, "对不起，您的等级未达到30级，不能领取秦皇战神BUFF。" )
		EndEvent(sceneId)
	...
	if LuaFnHaveImpactOfSpecificDataIndex( sceneId, selfId, x889050_g_BufferId ) == 1 then
		BeginEvent(sceneId)
			 AddText( sceneId, "您已经领取过秦皇战神BUFF，请在BUFF效果结束后再行领取。" )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end	
	...
	BeginEvent(sceneId)
			AddText( sceneId, "恭喜您，成功领取到秦皇战神Buff。" )
	EndEvent(sceneId)
	DispatchMissionTips( sceneId, selfId )
