...
-- 缝纫 9 级
-- 该脚本有以下三个必备功能函数：
-- x700940_AbilityCheck ——技能使用检查函数
-- x700940_AbilityConsume ——合成结束，进行相关消耗
-- x700940_AbilityProduce ——合成成功，产出产品
...
-- 配方等级(需求技能的等级)
x700940_g_RecipeLevel = 9

-- 材料表
x700940_g_CaiLiao = {
	{ID = 20105009, Count = 19},
	{ID = 20107011, Count = 21},
	{ID = 20108105, Count = 30},
	{ID = 20308078, Count = 1},
}
...
function x700940_AbilityProduce(sceneId, selfId)
	...
	for i, item in x700940_g_ChanPin do
		if item.Odds >= rand then
			Quality = CallScriptFunction( ABILITYLOGIC_ID, "CalcQuality", sceneId, x700940_g_RecipeLevel, AbilityLevel, x700940_g_AbilityMaxLevel,item.ID )
			...
			LuaFnAuditAbility( sceneId, selfId, x700940_g_AbilityID, x700940_g_RecipeID, Quality )
			LuaFnSendAbilitySuccessMsg( sceneId, selfId, x700940_g_AbilityID, x700940_g_RecipeID, item.ID )
			return OR_OK
		end
		LuaFnAuditAbility( sceneId, selfId, x700940_g_AbilityID, x700940_g_RecipeID, 0 )
	end
