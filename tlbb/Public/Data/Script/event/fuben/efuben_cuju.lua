...
x402040_g_LimitMembers = CUJU_MIN_ENTRY_PLAYER_COUNT				--可以进副本的最小队伍人数
...
function x402040_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnHasTeam(sceneId,selfId) ~= 1  then
		BeginEvent(sceneId)
			AddText(sceneId,"#B横扫牡丹腕");
			AddText(sceneId,"  横扫牡丹腕蹴鞠大赛需要"..CUJU_MIN_ENTRY_PLAYER_COUNT.."人以上组队才能参加，如果你只是想展示个人的才华，请去参加华山论剑吧！");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	-- 2，检测玩家是不是组队了
	if GetTeamSize(sceneId,selfId) < CUJU_MIN_ENTRY_PLAYER_COUNT  then
		BeginEvent(sceneId)
			AddText(sceneId,"#B横扫牡丹腕");
			AddText(sceneId,"  一支队伍不足"..CUJU_MIN_ENTRY_PLAYER_COUNT.."人，就算是进入蹴鞠场也没有什么获胜的可能啊，还是不要去了。");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
...
function x402040_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "cuju.nav"); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
