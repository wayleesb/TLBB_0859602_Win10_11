...
-- 缝纫 8 级
-- 该脚本有以下三个必备功能函数：
-- x700955_AbilityCheck ——技能使用检查函数
-- x700955_AbilityConsume ——合成结束，进行相关消耗
-- x700955_AbilityProduce ——合成成功，产出产品
...
-- 配方等级(需求技能的等级)
x700955_g_RecipeLevel = 12

-- 材料表
x700955_g_CaiLiao = {
	{ID = 20105012, Count = 20},
	{ID = 20107018, Count = 22},
	{ID = 20108084, Count = 35},
	{ID = 20308070, Count = 1},
}
...
function x700955_AbilityProduce(sceneId, selfId)
	...
	for i, item in x700955_g_ChanPin do
		if item.Odds >= rand then
			Quality = CallScriptFunction( ABILITYLOGIC_ID, "CalcQuality", sceneId, x700955_g_RecipeLevel, AbilityLevel, x700955_g_AbilityMaxLevel,item.ID )
			...
			LuaFnAuditAbility( sceneId, selfId, x700955_g_AbilityID, x700955_g_RecipeID, Quality )
			LuaFnSendAbilitySuccessMsg( sceneId, selfId, x700955_g_AbilityID, x700955_g_RecipeID, item.ID )
			return OR_OK
		end
		LuaFnAuditAbility( sceneId, selfId, x700955_g_AbilityID, x700955_g_RecipeID, 0 )
	end
