...
-- 缝纫 8 级
-- 该脚本有以下三个必备功能函数：
-- x700938_AbilityCheck ——技能使用检查函数
-- x700938_AbilityConsume ——合成结束，进行相关消耗
-- x700938_AbilityProduce ——合成成功，产出产品
...
-- 配方等级(需求技能的等级)
x700938_g_RecipeLevel = 8

-- 材料表
x700938_g_CaiLiao = {
	{ID = 20105008, Count = 16},
	{ID = 20107009, Count = 18},
	{ID = 20108116, Count = 26},
	{ID = 20308098, Count = 1},
}
...
function x700938_AbilityProduce(sceneId, selfId)
	...
	for i, item in x700938_g_ChanPin do
		if item.Odds >= rand then
			Quality = CallScriptFunction( ABILITYLOGIC_ID, "CalcQuality", sceneId, x700938_g_RecipeLevel, AbilityLevel, x700938_g_AbilityMaxLevel,item.ID )
			...
			LuaFnAuditAbility( sceneId, selfId, x700938_g_AbilityID, x700938_g_RecipeID, Quality )
			LuaFnSendAbilitySuccessMsg( sceneId, selfId, x700938_g_AbilityID, x700938_g_RecipeID, item.ID )
			return OR_OK
		end
		LuaFnAuditAbility( sceneId, selfId, x700938_g_AbilityID, x700938_g_RecipeID, 0 )
	end
