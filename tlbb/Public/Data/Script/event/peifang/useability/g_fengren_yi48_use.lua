...
-- 缝纫 5 级
-- 该脚本有以下三个必备功能函数：
-- x700924_AbilityCheck ——技能使用检查函数
-- x700924_AbilityConsume ——合成结束，进行相关消耗
-- x700924_AbilityProduce ——合成成功，产出产品
...
-- 配方等级(需求技能的等级)
x700924_g_RecipeLevel = 5

-- 材料表
x700924_g_CaiLiao = {
	{ID = 20105005, Count = 13},
	{ID = 20107005, Count = 15},
	{ID = 20108101, Count = 18},
	{ID = 20308075, Count = 1},
}
...
function x700924_AbilityProduce(sceneId, selfId)
	...
	for i, item in x700924_g_ChanPin do
		if item.Odds >= rand then
			Quality = CallScriptFunction( ABILITYLOGIC_ID, "CalcQuality", sceneId, x700924_g_RecipeLevel, AbilityLevel, x700924_g_AbilityMaxLevel,item.ID )
			...
			LuaFnAuditAbility( sceneId, selfId, x700924_g_AbilityID, x700924_g_RecipeID, Quality )
			LuaFnSendAbilitySuccessMsg( sceneId, selfId, x700924_g_AbilityID, x700924_g_RecipeID, item.ID )
			return OR_OK
		end
		LuaFnAuditAbility( sceneId, selfId, x700924_g_AbilityID, x700924_g_RecipeID, 0 )
	end
