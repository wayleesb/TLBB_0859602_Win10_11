...
-- 缝纫 3 级
-- 该脚本有以下三个必备功能函数：
-- x700916_AbilityCheck ——技能使用检查函数
-- x700916_AbilityConsume ——合成结束，进行相关消耗
-- x700916_AbilityProduce ——合成成功，产出产品
...
-- 配方等级(需求技能的等级)
x700916_g_RecipeLevel = 3

-- 材料表
x700916_g_CaiLiao = {
	{ID = 20105003, Count = 9},
	{ID = 20107003, Count = 11},
	{ID = 20108099, Count = 10},
	{ID = 20308073, Count = 1},
}
...
function x700916_AbilityProduce(sceneId, selfId)
	...
	for i, item in x700916_g_ChanPin do
		if item.Odds >= rand then
			Quality = CallScriptFunction( ABILITYLOGIC_ID, "CalcQuality", sceneId, x700916_g_RecipeLevel, AbilityLevel, x700916_g_AbilityMaxLevel,item.ID )
			...
			LuaFnAuditAbility( sceneId, selfId, x700916_g_AbilityID, x700916_g_RecipeID, Quality )
			LuaFnSendAbilitySuccessMsg( sceneId, selfId, x700916_g_AbilityID, x700916_g_RecipeID, item.ID )
			return OR_OK
		end
		LuaFnAuditAbility( sceneId, selfId, x700916_g_AbilityID, x700916_g_RecipeID, 0 )
	end
